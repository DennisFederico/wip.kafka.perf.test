...
echo "Start topic creation job($JOB_COMPLETION_INDEX) of '$TOPIC_COUNT' topics with prefix '$TOPIC_PREFIX-$JOB_COMPLETION_INDEX' ..."
...
for ((i=1; i<=TOPIC_COUNT; i++)); do
    TOPIC_NAME="${TOPIC_PREFIX}-${JOB_COMPLETION_INDEX}-${i}"
    kafka-topics.sh \
